...
int main (int argc, char *argv[])
{
  ...
  Config::SetDefault("ns3::RateErrorModel::ErrorUnit", StringValue("ERROR_UNIT_PACKET"));
   ...
   if(em == nullptr){
   std::cout<< "null was here" << std::endl;
   }
 ...
 // p2ph.SetDeviceAttribute ("ReceiveErrorModel", PointerValue (&error_model));
  ...
  PointToPointHelper p2phEnb;
  p2phEnb.SetDeviceAttribute ("DataRate", DataRateValue (internet_bandwidth));
  p2phEnb.SetChannelAttribute ("Delay", TimeValue (delay_enb));
  //p2phEnb.SetDeviceAttribute ("ReceiveErrorModel", PointerValue (&error_model));
  NetDeviceContainer internetDevicesEnb = p2phEnb.Install ( enbNodes.Get(0),sgw);
  //internetDevicesEnb.Get(1)->SetAttribute("ReceiveErrorModel", PointerValue(em));
 ...
 MobilityHelper enbMob;
  ...
  //lteHelper->SetFadingModel("ns3::NakagamiPropagationLossModel");
  //lteHelper->SetFadingModelAttribute ("m1", StringValue("1.0")); 
  ...
  for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
    {
      ...
      BulkSendHelper source ("ns3::TcpSocketFactory", InetSocketAddress (ueIpIface.GetAddress(u), dlPort));
